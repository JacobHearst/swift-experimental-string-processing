...
extension MEProgram.Builder {
  ...
  mutating func buildAdvance(_ n: Distance, reverse: Bool) {
    let opcode = reverse ? Instruction.OpCode.reverse : .advance
    instructions.append(.init(opcode, .init(distance: n)))
  }
  
  mutating func buildAdvanceUnicodeScalar(_ n: Distance, reverse: Bool) {
    let opcode = reverse ? Instruction.OpCode.reverse : .advance
    instructions.append(
      .init(opcode, .init(distance: n, isScalarDistance: true)))
  }
  ...
  mutating func buildMatchBuiltin(model: _CharacterClassModel, reverse: Bool) {
    ...
    instructions.append(.init(opcode, .init(model)))
  }
